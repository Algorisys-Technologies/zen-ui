...
const Card = (props: { binding: Binding }) => {
  ...
  return (
    ...
    >
      ...
      <Show when={b().demoHref}>
        <a href={b().demoHref} class="block mb-4 no-underline">
          {/* A screenshot of the demo this card links to, generated by
              `bun run gen:previews` and regenerated on every deploy, so it
              cannot drift from what the link actually opens.
              The images are gitignored, so the <img> removes itself when one is
              missing and the card is exactly what it was before — a broken
              image on a marketing page is worse than no image. */}
          <img
            src={`${import.meta.env.BASE_URL}previews/${b().demoHref!.replace(/\/$/, "").split("/").pop()}.jpg`}
            alt=""
            aria-hidden
            loading="lazy"
            class="block w-full rounded-zen-md border border-zen-border bg-zen-muted object-cover object-top"
