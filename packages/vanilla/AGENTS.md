...
# @algorisys/zen-ui-vanilla — for AI coding agents

The **Vanilla** binding of zen-ui. Framework-free factories. Each factory takes a props object and returns a handle whose `.el` is the DOM node. No JSX, no compound child components — a family is one data-driven factory.

zen-ui ships four bindings that share one API and one design core; this file is
for agents targeting **Vanilla**. The other bindings —
`@algorisys/zen-ui-react`, `@algorisys/zen-ui-solid`, `@algorisys/zen-ui-web-components` — expose the same catalogue with their own idiom.
