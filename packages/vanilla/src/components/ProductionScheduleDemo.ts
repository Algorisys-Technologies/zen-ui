...
function reschedulable(): HTMLElement {
  ...
  const chart = ProductionSchedule({
    resources: CELLS,
    operations: jobs,
...
ProductionSchedule({ resources: cells, operations: jobs, calendar: plant }).el`,
        render: () =>
          ProductionSchedule({ resources: CELLS, operations: JOBS, dependencies: ROUTING, calendar: PLANT, now: NOW }).el,
      },
      {
...
ProductionSchedule({ resources: cells, operations: jobs,
                     calendar: plant, maxLanes: 3, showLoad: true }).el`,
        render: () =>
          ProductionSchedule({
            resources: SQUEEZED,
...
const chart = ProductionSchedule({
  ...
  onReschedule: (proposal) => {
    ...
    chart.update({ operations: jobs });   // the caller re-renders it
  },
...
ProductionSchedule({
  ...
  onOperationClick: (op) => openJobCard(op),
      ...
      },
      {
        ...
        code: `ProductionSchedule({ resources: [], operations: [], loading: true }).el
ProductionSchedule({ resources: [], operations: [] }).el`,
        render: () => {
          const wrap = document.createElement("div");
          wrap.className = "zen-flex zen-w-full zen-flex-col zen-gap-4";
          wrap.append(
            ProductionSchedule({ resources: [], operations: [], loading: true, now: NOW }).el,
            ProductionSchedule({ resources: [], operations: [], now: NOW }).el,
          );
          return wrap;
        },
      },
