    ...
    sections: [
      {
        ...
        codeDescription:
          "A URL you already have the right to read, or a Blob you already hold. It does not fetch: no method, no headers, no retry policy, for the same reason UploadCollection has none. A signed URL works untouched, because its query string is stripped before the extension is read — skipping that step is what renders every document in an app as a broken image.",
        code: `DocumentViewer({ src: signedUrl, name: "grn-2026-0431.png" }).el`,
        render: () =>
          DocumentViewer({ src: SAMPLE_IMAGE, name: "grn-2026-0431.svg", height: "26rem" }).el,
      },
      {
        title: "2. A PDF, with pages",
        codeTitle: "Rendered to a canvas, not dropped in an iframe",
        codeDescription:
          "An iframe hands the browser's own viewer a frame box and lets it lay out inside — so the page count is unreadable, rotation is impossible, and zoom crops instead of magnifying unless you inflate the frame by 1/zoom and scale it back, which behaves differently in all three engines. Rendering the page ourselves makes zoom a render parameter and rotation a number. The canvas is drawn at device resolution so a 2x display shows the document rather than a blur of it.",
        code: `import workerSrc from "pdfjs-dist/build/pdf.worker.min.mjs?url";

DocumentViewer({ src: pdfUrl, name: "invoice-8842.pdf", workerSrc }).el`,
        render: () =>
          DocumentViewer({
            src: SAMPLE_PDF,
  ...
  onPageChange: (p) => (page = p),
        ...
        render: () => {
          ...
          let page = 1;
          const paint = () => {
            readout.textContent = `zoom ${Math.round(zoom * 100)}% · page ${page}`;
          };
          paint();
      ...
      },
      {
        ...
        codeDescription:
          "No onDownload, no download button. A disabled-looking control that does nothing is worse than an absent one, because the user spends a click finding out. toolbar: false drops the bar entirely, for a thumbnail-sized preview where the controls would outweigh the document.",
        code: `DocumentViewer({ src, onDownload: (src, name) => save(src, name) }).el
DocumentViewer({ src, toolbar: false, height: "12rem" }).el`,
        render: () => {
          ...
          stack.append(
            ...
            DocumentViewer({ src: SAMPLE_IMAGE, toolbar: false, height: "12rem" }).el,
          );
          return stack;
        },
      },
      {
        ...
        code: `DocumentViewer({ src: "/exports/ledger.zip", name: "ledger.zip", onDownload: save }).el`,
        render: () =>
          DocumentViewer({
