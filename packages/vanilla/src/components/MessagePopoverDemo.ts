    ...
    { id: "1", type: "error", title: "VAT number is not valid",
      subtitle: "VAT number", targetId: "vat" },
        ...
        render: () => {
          ...
          row.append(MessagePopover({ messages: MESSAGES }).el, hint);
          wrap.append(row, fieldGrid((id, label) => Input({ id, placeholder: label }).el));
          return [wrap];
        },
      },
      {
...
MessagePopover({ messages });`,
        render: () => [MessagePopover({ messages: MESSAGES }).el],
      },
      {
        ...
        render: () => [MessagePopover({ messages: SEVERITY_SAMPLE }).el],
      },
      {
        title: "4. Empty",
        ...
        render: () => [MessagePopover({ messages: [], emptyMessage: "Nothing to fix" }).el],
      },
    ],
  });
}
