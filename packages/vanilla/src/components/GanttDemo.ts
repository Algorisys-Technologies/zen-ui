...
/** Section 2: collapsing a parent, driven from outside the factory. */
function collapsible(): HTMLElement {
  ...
  const chart = Gantt({
    ...
    onExpandedChange: (ids) => {
      out.textContent = `Open: ${ids.length === 0 ? "nothing" : ids.join(", ")}`;
      /* Controlled here, so the caller pushes the new set back in — the same
         shape as passing `expanded` from a store in the JSX bindings. */
      chart.update({ expanded: ids });
    },
  });
  wrap.append(chart.el, out);
  return wrap;
}
...
function clickable(): HTMLElement {
  ...
  const chart = Gantt({
    ...
    onTaskClick: (task, row) => {
      out.textContent = `${task.name} — ${row.status}${row.progress === null ? "" : `, ${Math.round(row.progress)}%`}`;
    },
  });
  wrap.append(chart.el, out);
  return wrap;
...
function loadingAndEmpty(): HTMLElement {
  const wrap = document.createElement("div");
...
Gantt({ tasks: plan, defaultView: "month" }).el`,
        render: () =>
...
Gantt({ tasks: plan, dependencies: links, showDependencies: false }).el`,
        render: () =>
          Gantt({
            tasks: PLAN,
            dependencies: LINKS,
...
Gantt({ tasks: plan, views: ["week", "month"], defaultView: "week" }).el`,
        render: clickable,
      },
      {
...
Gantt({ tasks: [], emptyState: myOwnEmpty() }).el`,
        render: loadingAndEmpty,
      },
    ],
  });
}
