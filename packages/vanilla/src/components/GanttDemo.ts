...
Gantt({ tasks: plan }).el`,
        render: () => Gantt({ tasks: PLAN, dependencies: LINKS, now: NOW }).el,
      },
      {
        ...
        code: `Gantt({ tasks: plan }).el                       // fit is the default
Gantt({ tasks: plan, defaultView: "month" }).el // or pin a calendar window`,
        render: () => {
          const wrap = document.createElement("div");
          wrap.className = "zen-flex zen-w-full zen-flex-col zen-gap-4";
          wrap.append(
            Gantt({ tasks: CROSS_YEAR, now: NOW }).el,
            Gantt({ tasks: CROSS_YEAR, now: NOW, defaultDate: NOW, defaultView: "year" }).el,
          );
          return wrap;
        },
      },
      {
        ...
        code: `const chart = Gantt({
  tasks: plan,
  dependencies: links,
  expanded: ["p1", "p2", "p3"],
  onExpandedChange: (ids) => chart.update({ expanded: ids }),
});`,
        render: () =>
...
Gantt({ tasks: plan, dependencies: links }).el`,
  ...
  baselineEnd: at(8) }        // -> "+2d", and a delayed bar until it hits 100%`,
        render: () =>
          Gantt({ tasks: PLAN, dependencies: LINKS, now: NOW, defaultDate: NOW, defaultView: "month" }).el,
      },
      {
        ...
        code: `Gantt({ tasks: plan, dependencies: links, defaultView: "year" }).el
Gantt({ tasks: plan, defaultView: "quarter" }).el`,
        render: () => {
          const wrap = document.createElement("div");
          wrap.className = "zen-flex zen-w-full zen-flex-col zen-gap-4";
          wrap.append(
            Gantt({ tasks: LONG_PLAN, dependencies: LONG_LINKS, now: NOW, defaultDate: NOW, defaultView: "year" }).el,
            Gantt({ tasks: LONG_PLAN, dependencies: LONG_LINKS, now: NOW, defaultDate: NOW, defaultView: "quarter" }).el,
          );
          return wrap;
        },
      },
      {
        ...
        code: `Gantt({ tasks: plan, columns: ["name", "status"] }).el
Gantt({ tasks: plan }).el   // or let the container decide`,
        render: () => {
          const wrap = document.createElement("div");
          wrap.className = "zen-flex zen-w-full zen-flex-col zen-gap-4";
          const narrow = document.createElement("div");
          narrow.style.maxWidth = "800px";
          narrow.append(Gantt({ tasks: LONG_PLAN, dependencies: LONG_LINKS, now: NOW }).el);
          wrap.append(
            Gantt({ tasks: LONG_PLAN, dependencies: LONG_LINKS, now: NOW, columns: ["name", "status"] }).el,
            narrow,
          );
          return wrap;
        },
      },
      {
        ...
        code: `Gantt({ tasks: plan, onTaskClick: (task) => open(task) }).el
        ...
        render: () =>
          withReadout("Tab to the chart, then use the arrow keys", (say) =>
            Gantt({
              ...
              now: NOW,
              onTaskClick: (task) => say(`Activated: ${task.name}`),
            }).el,
          ),
      },
      {
...
{ id: "j2", name: "Heat treat", start: at(24, 16), workingMinutes: 360 }`,
        render: () => {
          ...
          wrap.className = "zen-flex zen-w-full zen-flex-col zen-gap-4";
          wrap.append(
            Gantt({ tasks: SHOP, dependencies: SHOP_LINKS, calendar: PLANT, now: SHOP_NOW, defaultDate: SHOP_NOW, defaultView: "week" }).el,
            Gantt({ tasks: SHOP, dependencies: SHOP_LINKS, calendar: PLANT, now: SHOP_NOW, defaultDate: SHOP_NOW, defaultView: "day" }).el,
            // The four-hour overtime job on its own, so fit lands in the HOUR
            // band — the resolution a shop floor actually schedules at.
            Gantt({ tasks: [SHOP[0].children![3]], calendar: PLANT, now: SHOP_NOW }).el,
          );
          return wrap;
        },
      },
      {
        ...
        code: `Gantt({ tasks: [], loading: true }).el
Gantt({ tasks: [] }).el                     // the default empty surface`,
        render: () => {
          const wrap = document.createElement("div");
          wrap.className = "zen-flex zen-w-full zen-flex-col zen-gap-4";
          wrap.append(
            Gantt({ tasks: [], loading: true, now: NOW, defaultDate: NOW }).el,
            Gantt({ tasks: [], now: NOW, defaultDate: NOW }).el,
          );
          return wrap;
        },
      },
...
// PageDown      one screenful at a time`,
        render: () => Gantt({ tasks: BIG_PLAN, now: NOW, defaultExpanded: ["w0", "w1"] }).el,
      },
    ],
  });
}
