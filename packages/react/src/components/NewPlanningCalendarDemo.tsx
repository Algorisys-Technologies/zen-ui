...
import { PlanningCalendar, type PlanningRow } from "./planning-calendar/planning-calendar";
...
const NewPlanningCalendarDemo = () => (
  <div className="demo-page">
    ...
    <section className="demo-section">
      <h2>1. Resources down, time across</h2>
      <CodeExample
        title="A row is a person, a room or a machine"
...
<PlanningCalendar rows={rows} />`}
    ...
    <section className="demo-section">
      <h2>2. Three views, one axis</h2>
      <CodeExample
        title="A month is 31 columns, not a 6×7 page"
...
<PlanningCalendar rows={rows} view={view()} onViewChange={setView} />`}
    ...
    <section className="demo-section">
      <h2>3. Overlaps stack; edges are cut, not squared off</h2>
      <CodeExample
        title="The day view, where both actually show"
        description="Appointments that overlap are assigned lanes and stack, using exactly as many lanes as the busiest instant needs — so a double-booking is visible rather than hidden behind whichever block drew last. Back-to-back is not overlap: 10:00–11:00 and 11:00–12:00 share a lane, or a normal day would become a staircase. An appointment that starts before the view or ends after it is clipped, and the cut edge loses its border and corner so it reads as continuing rather than ending there. A zero-length appointment still gets a clickable sliver, because a 0%-wide block is indistinguishable from data that failed to load."
...
/>`}
    ...
    <section className="demo-section">
      ...
      <CodeExample
        title="emptyMessage"
        description="No resources means there is nothing to compare, so the grid is not drawn at all — an empty axis with no rows under it reads as a component that failed to load."
