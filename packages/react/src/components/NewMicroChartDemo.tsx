...
<MicroRadialChart value={72} showValue />`}
        >
          <div className="zen-flex zen-flex-wrap zen-items-center zen-gap-8">
            {[
              ["line", <MicroLineChart values={REVENUE} />],
              ["bar", <MicroBarChart values={REVENUE} />],
              ["bullet", <MicroBulletChart value={82} target={75} />],
              ["delta", <MicroDeltaChart from={40} to={52} />],
              ["radial", <MicroRadialChart value={72} showValue />],
            ].map(([name, chart]) => (
              <div key={name as string} className="zen-flex zen-flex-col zen-items-center zen-gap-1">
                {chart as React.ReactNode}
                <span className="zen-text-xs zen-text-zen-muted-fg">{name as string}</span>
              </div>
            ))}
          </div>
        </CodeExample>
      ...
      <section className="demo-section">
        <h2>2. Delta colours itself</h2>
        <CodeExample
          title={'Direction is the point, so the caller does not paint it'}
          description={'A rise is success and a fall is error, derived from the numbers rather than passed in — letting a caller paint a fall green would defeat the component. Pass color explicitly only when up is bad: cost, error rate, latency, churn. That is the one case where the default would lie.'}
...
<MicroDeltaChart from={2.1} to={3.4} color="error" />`}
        >
          <div className="zen-flex zen-flex-wrap zen-items-end zen-gap-8">
            <div className="zen-flex zen-flex-col zen-items-center zen-gap-1">
              <MicroDeltaChart from={40} to={52} />
              <span className="zen-text-xs zen-text-zen-muted-fg">40 → 52</span>
            </div>
            <div className="zen-flex zen-flex-col zen-items-center zen-gap-1">
              <MicroDeltaChart from={55} to={47} />
              <span className="zen-text-xs zen-text-zen-muted-fg">55 → 47</span>
            </div>
            <div className="zen-flex zen-flex-col zen-items-center zen-gap-1">
              <MicroDeltaChart from={2.1} to={3.4} color="error" />
              <span className="zen-text-xs zen-text-zen-muted-fg">churn up (bad)</span>
            </div>
          </div>
        </CodeExample>
      </section>
      ...
      <section className="demo-section">
        <h2>3. Where they actually go</h2>
        <CodeExample
          title={'Inside a table cell'}
          description={"This is the case micro charts exist for. They are sized in px and sit in text flow, so they align on the baseline of the row without stretching the cell. A percentage width would collapse in a cell that has not been measured, which is why there isn't one."}
...
<td><MicroBulletChart value={row.quota} target={75} width={64} /></td>`}
        >
          <table className="zen-w-full zen-text-sm">
            <thead>
              <tr className="zen-border-b zen-border-zen-border zen-text-xs zen-text-zen-muted-fg">
                <th className="zen-px-2 zen-py-2 zen-text-start">Region</th>
                <th className="zen-px-2 zen-py-2 zen-text-start">Trend</th>
                <th className="zen-px-2 zen-py-2 zen-text-start">Quota</th>
                <th className="zen-px-2 zen-py-2 zen-text-start">Change</th>
              </tr>
            </thead>
            <tbody>
              {ROWS.map((r) => (
                <tr key={r.name} className="zen-border-b zen-border-zen-border last:zen-border-0">
                  <td className="zen-px-2 zen-py-3">{r.name}</td>
...
<span className="zen-text-zen-warning"><MicroBarChart values={data} /></span>`}
      ...
      <section className="demo-section">
        <h2>5. Screen readers get the number</h2>
        <CodeExample
          title={"role='img' with a derived label"}
