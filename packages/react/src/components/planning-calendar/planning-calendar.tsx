...
export const PlanningCalendar = ({
  ...
  /* Read once per render rather than per call site: two `new Date()`s in one
     render can straddle a millisecond, and the marker and the today column
     would then disagree about which one it is. */
  ...
  /* Placement and lanes are computed once per row rather than inside the block
     loop, which would re-run the whole lane assignment for every appointment —
     O(n²) on a row with a busy day. */
  ...
  return (
    ...
    <div className={cn("zen-flex zen-w-full zen-flex-col zen-gap-3", className)}>
      {/* The toolbar is gated on there being rows, not only on `hideToolbar`.
          ...
          an empty resource list is the same empty list. */}
      {!hideToolbar && laidOut.length > 0 && (
        <div className="zen-flex zen-flex-wrap zen-items-center zen-gap-2">
          ...
          >
            {/* Logical, not physical: under RTL the axis runs the other way, so
                a left-pointing chevron would send you forward. */}
      ...
      {laidOut.length === 0 ? (
        ...
        <div className="zen-overflow-x-auto zen-rounded-zen-md zen-border zen-border-zen-border">
          <div className="zen-min-w-[45rem]">
            <div className="zen-flex zen-border-b zen-border-zen-border zen-bg-zen-muted/30">
              <div className="zen-w-40 zen-shrink-0 zen-border-e zen-border-zen-border zen-px-3 zen-py-2 zen-text-xs zen-font-semibold zen-text-zen-muted-fg">
                Resource
              </div>
              <div className="zen-flex zen-flex-1">
                {columns.map((column) => (
                  <div
                  ...
                  >
                    <div className="zen-text-xs zen-font-medium zen-text-zen-foreground">
                      {column.label}
                    </div>
                    {column.sublabel && (
                      <div className="zen-text-[10px] zen-text-zen-muted-fg">{column.sublabel}</div>
                    )}
                  </div>
                ))}
              </div>
            ...
            {laidOut.map((entry) => (
              ...
              >
                <div className="zen-w-40 zen-shrink-0 zen-border-e zen-border-zen-border zen-px-3 zen-py-2">
                  <div className="zen-truncate zen-text-sm zen-font-medium zen-text-zen-foreground">
                    {entry.row.title}
                  </div>
                  {entry.row.subtitle && (
                    <div className="zen-truncate zen-text-xs zen-text-zen-muted-fg">
                      {entry.row.subtitle}
                    </div>
                  )}
                </div>
                ...
                <div
                  ...
                  style={{ minHeight: `${entry.laneCount * LANE_PX + 8}px` }}
                >
                  {/* The column rules, drawn as a background layer rather than as
                      parents of the blocks: a block spanning four hours cannot
                      live inside one column's box. */}
                  ...
                  {entry.blocks.map((block) => (
                    <button
                      ...
                      type="button"
                      onClick={() => onAppointmentClick?.(block.appointment, entry.row)}
                      /* A button whether or not a handler was passed: these are
                         the only things in the grid worth reaching by keyboard,
                         and a div with no tabindex takes the whole calendar out
                         of the tab order. Without a handler it is inert but
                         still focusable and still readable. */
                      className={cn(
                        ...
                        onAppointmentClick && "hover:zen-brightness-95",
                      )}
                      style={{
                        insetInlineStart: `${block.placement.startPct}%`,
                    ...
                    >
                      {/* The time is in the accessible name, not only the title
                          attribute: a block narrow enough to drop its own label
                          is exactly when someone needs it, and title is not
                          reliably announced. */}
                      <span className="zen-sr-only">
                        {formatTimeRange(block.appointment.start, block.appointment.end)}
                      </span>
                      {block.placement.widthPct >= TEXT_MIN_WIDTH_PCT && (
                        <>
                          {block.appointment.icon && (
                            <Icon name={block.appointment.icon} size={12} className="zen-shrink-0" />
                          )}
                          <span className="zen-truncate zen-font-medium">
                            {block.appointment.title}
                          </span>
                          {block.appointment.subtitle && (
                            <span className="zen-truncate zen-opacity-70">
                              {block.appointment.subtitle}
                            </span>
                          )}
                        </>
