...
# @algorisys/zen-ui-react — for AI coding agents

The **React** binding of zen-ui. JSX components (Radix-backed). This is the reference API.

zen-ui ships four bindings that share one API and one design core; this file is
for agents targeting **React**. The other bindings —
`@algorisys/zen-ui-solid`, `@algorisys/zen-ui-vanilla`, `@algorisys/zen-ui-web-components` — expose the same catalogue with their own idiom.
