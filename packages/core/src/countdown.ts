 ...
 */
export const DEFAULT_COUNTDOWN_THRESHOLDS: readonly number[] = [300, 60];
 ...
 */
export const countdownLevel = (
  ms: number,
  thresholds: readonly number[] = DEFAULT_COUNTDOWN_THRESHOLDS,
): CountdownLevel => {
  if (ms <= 0) return "expired";
  const seconds = ms / 1000;
  const [warning, critical] = thresholds;
  if (critical !== undefined && seconds <= critical) return "critical";
  if (warning !== undefined && seconds <= warning) return "warning";
  return "normal";
};
...
export const crossedThresholds = (
  ...
  const prev = previousMs / 1000;
  const now = currentMs / 1000;
  return thresholds
    .filter((mark) => prev > mark && now <= mark)
    .sort((a, b) => b - a);
};
