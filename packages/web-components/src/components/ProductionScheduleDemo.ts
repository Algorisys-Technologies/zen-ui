...
/**
 * Every chart below is a <zen-production-schedule>, built the way a framework
 * template binds one: attributes for the primitives, PROPERTIES for the trees
 * and the callbacks. `canReschedule` is a predicate, so it is a property only —
 * there is no attribute form of a function and there should not be one.
 */
type ScheduleEl = HTMLElement & Record<string, unknown>;

const schedule = (props: Record<string, unknown>): ScheduleEl => {
  const el = document.createElement("zen-production-schedule") as ScheduleEl;
 ...
 *
 * The no-framework version of the same argument, and it is the most literal of
 * the three: there is no signal to write to, so "the caller re-renders it" is a
 * call to `update()` on a handle this function holds. Nothing about the
 * component changed to make that work — it never had internal operation state
 * to begin with, which is exactly what the contract claims.
 */
function reschedulable(): HTMLElement {
  const wrap = document.createElement("div");
  wrap.className = "zen-flex zen-w-full zen-min-w-0 zen-flex-col zen-gap-2";

  let jobs: ProductionOperation[] = JOBS;
  const history: ProductionOperation[][] = [];

  const bar = document.createElement("div");
  bar.className = "zen-flex zen-items-center zen-gap-3";
  const note = document.createElement("p");
  note.className = "zen-m-0 zen-text-sm zen-text-zen-muted-fg";
  note.setAttribute("aria-live", "polite");
  note.textContent = "Drag a bar, or focus one and press Alt + arrow.";

  const sync = () => {
    undo.update({
      disabled: history.length === 0,
      children: history.length > 0 ? `Undo (${history.length})` : "Undo",
    });
  };
  ...
  const chart = schedule({
    ...
    canReschedule: (operation: ProductionOperation) => operation.resourceId !== "insp",
  });

  bar.append(undo.el, note);
  wrap.append(chart, bar);
  return wrap;
}

/** Section 7: a live click handler, so the section is not a dead control. */
function clickable(): HTMLElement {
  const wrap = document.createElement("div");
  wrap.className = "zen-flex zen-w-full zen-min-w-0 zen-flex-col zen-gap-2";
  const picked = document.createElement("p");
  picked.className = "zen-m-0 zen-text-sm zen-text-zen-muted-fg";
  picked.setAttribute("aria-live", "polite");
  picked.textContent = "Nothing picked yet";
  wrap.append(
    schedule({
      resources: CELLS,
      operations: JOBS,
      ...
      calendar: PLANT,
      now: NOW,
      onOperationClick: (operation: ProductionOperation) => {
        picked.textContent = `${operation.name}${operation.order ? ` · ${operation.order}` : ""}`;
      },
    }),
    picked,
  );
...
{ "id": "j3", "resourceId": "cnc3", "name": "Bracket · op 10",
  "start": "2026-07-21T16:30", "setupMinutes": 45, "runMinutes": 360 }`,
        render: () =>
          schedule({
...
chart.addEventListener("zen-operation-click", (e) => openJobCard(e.detail[0]));`,
      ...
      },
      {
        ...
        codeDescription:
          "The skeleton draws two blocks per row rather than one long bar, because a machine's day is a sequence and a single bar reads as the wrong component for a second. The toolbar is not drawn over either state: Previous, Today and Next cannot change anything the user can see when there is nothing to see.",
        code: `<zen-production-schedule resources="[]" operations="[]" loading></zen-production-schedule>
<zen-production-schedule resources="[]" operations="[]"></zen-production-schedule>`,
        render: () => {
          const wrap = document.createElement("div");
          wrap.className = "zen-flex zen-w-full zen-flex-col zen-gap-4";
          wrap.append(
            schedule({ resources: [], operations: [], loading: true, now: NOW }),
            schedule({ resources: [], operations: [], now: NOW }),
          );
          return wrap;
        },
      },
