...
const PLAIN: TimelineItem[] = ORDER.map(({ group: _g, ...rest }) => rest);
...
const NewTimelineDemo = () => (
  <DemoPage
    ...
    description={
      <>
        A sequence of things that happened, in order — an audit trail, an
        order's history, a ticket's comments. Renders an ordered list, because
        the order <em>is</em> the content.
      </>
    }
  >
    <DemoSection
...
<Timeline items={items} />`}
    ...
    <DemoSection
      ...
      codeDescription="A run of items sharing a group gets one heading. It is a string you set rather than a function that derives it, because you already know whether two events fall on the same day — deriving it here would mean guessing at your timezone and your idea of 'today', and getting that wrong is worse than not offering it. The heading is not a list item, so it does not inflate the count a screen reader announces."
      code={`const items = [
...
{ id: "3", title: "Departed" }                        // default`}
    ...
    <DemoSection
      ...
      codeDescription="A description is a line of prose; children is anything else — a diff, a quote, an attachment, a set of badges. It sits under the description and inherits the rail's indent, so a wide body still lines up with the rest of the column."
      code={`{
  id: "1",
  ...
  children: <DiffView before={before} after={after} />,
    ...
    >
      <div class="zen-max-w-xs zen-rounded-zen-md zen-border zen-border-zen-border zen-p-3">
